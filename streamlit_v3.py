...
🎯 COMPLETE FEATURE PARITY WITH COLAB ORIGINAL
...
📅 Updated: 2024-01
🔒 Logic: 100% Identical to Colab Original
...
# ==========================================
# 📄 FILE READING (COMPLETE ORIGINAL LOGIC)
# ==========================================
...
# ==========================================
# 🤖 AI TEXT FORMATTING (COMPLETE LOGIC)
# ==========================================
...
# ==========================================
# 🎯 CLASSIFICATION (COMPLETE ORIGINAL PATTERNS)
# ==========================================
...
def classify_scope_regex(sentences):
    ...
    for sent in sentences:
        ...
        scope_patterns = [
            ...
            r'(ระบบ|แพลตฟอร์ม|ซอฟต์แวร์|เครื่องมือ|แอป|เว็บแอพ).*(ต้อง|สามารถ|รองรับ|แสดงผล|จัดเก็บ|รวบรวม|สืบค้น|ค้นหา|วิเคราะห์|จำแนก|จัดหมวดหมู่|แจ้งเตือน|ส่งออก|นำเข้า|เชื่อมต่อ)',
            r'(ต้องมี|ต้องสามารถ|ต้องรองรับ|ต้องแสดงผล|ต้องจัดเก็บ|ต้องเชื่อมต่อ)',
            r'(Dashboard|แดชบอร์ด).*(เรียลไทม์|Online|รายงาน|แสดงผล)',
            r'(Realtime|เรียลไทม์|24\s*ชั่วโมง)',
            r'(รองรับ).*(ผู้ใช้งาน|Users|บัญชี|ช่องทาง|ภาษา)',
            r'(API|Webhook|SSO|Integration|เชื่อมต่อระบบ)',

            # TH: Contractor duties
            r'(ผู้รับจ้าง|ผู้ให้บริการ).*(ต้อง|ดำเนินการ|จัดหา|ติดตั้ง|พัฒนา|ออกแบบ|ทดสอบ|อบรม|ส่งมอบ)',
            r'(จัดทำ|นำเสนอ|ส่งมอบ).*(รายงาน|Dashboard|คู่มือ|เอกสาร)',
        ...
        non_scope_patterns = [
            ...
            r'(ความเป็นมา|วัตถุประสงค์|หลักการและเหตุผล)',
            r'(คุณสมบัติผู้เสนอ|ผลงาน|ประสบการณ์)',
            r'(งบประมาณ|วงเงิน|ราคากลาง)',
            r'(การชำระเงิน|งวด|ภาษีมูลค่าเพิ่ม)',
            r'(ค่าปรับ|เบี้ยปรับ|ค่าเสียหาย)',
...
def calculate_regex_confidence(sentence):
    ...
    high_conf_scope = [
        r'ระบบต้อง', r'ผู้รับจ้างต้อง',
        r'system must', r'platform shall', r'contractor must',
    ]

    high_conf_non_scope = [
        r'ค่าปรับ', r'การชำระเงิน', r'งบประมาณ',
...
🎯 MISSION: Classify sentences as "SCOPE" (technical requirements) or "NOT SCOPE" (admin/commercial content).

✅ MARK AS TRUE (Scope) when describing:
┌─────────────────────────────────────────────────────────────┐
│ 1. SYSTEM CAPABILITIES                                      │
│    • Thai: "ระบบต้อง...", "สามารถ...", "รองรับ...", "แสดงผล..." │
│    • English: "System must...", "Platform can...", "Supports..." │
│                                                             │
│ 2. CONTRACTOR DUTIES                                        │
│    • Thai: "ผู้รับจ้างต้อง...", "ดำเนินการ...", "วิเคราะห์..." │
│    • English: "Vendor must...", "Contractor shall...", "Analyze..." │
│                                                             │
│ 3. DELIVERABLES                                             │
│    • Thai: "รายงาน", "Dashboard", "คู่มือ", "เอกสาร"        │
│    • English: "Report", "Dashboard", "Manual", "Document"   │
└─────────────────────────────────────────────────────────────┘

❌ MARK AS FALSE (Not Scope) when describing:
┌─────────────────────────────────────────────────────────────┐
│ 1. BACKGROUND/INTRO                                         │
│    • Thai: "ความเป็นมา", "วัตถุประสงค์", "หลักการและเหตุผล" │
│    • English: "Background", "Introduction", "Objective"     │
│                                                             │
│ 2. COMMERCIAL/LEGAL TERMS                                   │
│    • Thai: "ค่าปรับ", "การชำระเงิน", "หลักประกัน", "งบประมาณ" │
│    • English: "Penalty", "Payment", "Guarantee", "Budget"   │
│                                                             │
│ 3. VENDOR QUALIFICATIONS                                    │
│    • Thai: "คุณสมบัติผู้เสนอ", "ผลงานอ้างอิง"              │
│    • English: "Bidder requirements", "Qualifications"       │
│                                                             │
│ 4. ADMINISTRATIVE                                           │
│    • Page numbers, Headers, Footers, Dates                 │
└─────────────────────────────────────────────────────────────┘

📚 EXAMPLES:
Input: ["ระบบต้องรวบรวมข้อมูล Social Media 5 แพลตฟอร์ม", "ค่าปรับ 0.1% ต่อวัน"]
Output: [true, false]
"""
...
OUTPUT:"""
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if consecutive_rate_limits >= 3:
                            ...
                            status_text.text(f"⏸️ Rate limit quota - pausing 60s...")
                            time.sleep(60)
        ...
        for is_scope in batch_results:
            results.append("✅ Scope" if is_scope else "")
...
def classify_scope_hybrid(sentences, api_key):
    ...
    for i, (sent, regex_result, confidence) in enumerate(zip(sentences, regex_results, confidences)):
        if confidence >= 0.9:
            results.append("✅ Scope" if regex_result else "")
        else:
    ...
    with st.spinner('🔍 Matching products...'):
        try:
            ...
            for i, sent in enumerate(tor_sentences):
                ...
                if final_score >= 65:
                    result_text = f"✅ {prod} ({int(final_score)}%)"
                    matched_products.append(prod)
                else:
                    result_text = f"❌ Miss ({int(final_score)}%)"
            ...
            st.error(f"❌ Error in product matching: {e}")
            return [], pd.DataFrame()

    with st.spinner('🧠 Classifying scope...'):
...
# ==========================================
# 💰 BUDGET ENGINE (COMPLETE ORIGINAL LOGIC)
# ==========================================
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["📤 Upload & Analyze", "📊 Results", "💰 Budget"])
        ...
        if uploaded_file and gemini_api_key:
            ...
            if st.button("🚀 Start Analysis", use_container_width=True):
                ...
                if uploaded_file.name.endswith(('.xlsx', '.xls')):
                    st.info("📊 Excel file detected: Skipping AI formatting")
                    tor_formatted = tor_raw
                else:
                    with st.spinner('🤖 AI formatting text...'):
                ...
                with st.spinner('💰 Extracting budget factors...'):
    ...
    with tab2:
        if st.session_state.analysis_complete:
            ...
            scope_count = len(st.session_state.result_df[st.session_state.result_df['Is_Scope?'] == '✅ Scope'])
            matched_count = len(st.session_state.result_df[st.session_state.result_df['Product_Match'].str.contains('✅', na=False)])
            products_found = len(st.session_state.matched_products)

            col1.metric("📝 Total Lines", total_sentences)
            col2.metric("✅ Scope Items", scope_count)
            col3.metric("🎯 Matched", matched_count)
            col4.metric("📦 Products", products_found)
            ...
            if st.session_state.matched_products:
                ...
                for prod in st.session_state.matched_products:
                    st.markdown(f'<span class="status-badge status-success">✅ {prod}</span>', unsafe_allow_html=True)
                st.markdown("---")
            ...
            if filter_scope == "Scope Only":
                df_filtered = df_filtered[df_filtered['Is_Scope?'] == '✅ Scope']
    ...
    with tab3:
        if st.session_state.analysis_complete and st.session_state.matched_products:
            ...
            with st.form("factor_form"):
                ...
                submitted = st.form_submit_button("🔄 Recalculate Budget")
